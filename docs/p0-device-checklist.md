...
| 3.4 | 已入库候选卡上出现「将写入行」预览 + 「写入看板」按钮(需已配置 `JOB_ONE_STOP_CONTEXT_REPO_PATH`),点击 | 按钮变「已写入看板」;在 Obsidian 打开 `toolkit/23-job-pipeline.md`,「收集箱」列多出预览里那一行,其它列与卡片原样不动,拖卡正常 |
| 3.5 | 对 3.4 已写入的候选再点一次(如按钮可达)或重发请求 | 幂等,看板不重复出现该行 |
...
- [ ] 3.4 写入看板成功且只动「收集箱」一行;3.5 幂等;未点「写入看板」前个人仓库零变化
