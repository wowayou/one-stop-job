...
- **决策聊天**：通用聊天 + 岗位专属聊天；支持文字、链接旁正文和截图，先运行硬规则，再由可选 AI 给出优先级、风险、唯一下一步和回复草稿
...
启动后访问 `GET /api/context/status` 检查只读连接。接口只返回白名单文件是否可用，不返回绝对路径、文件数量、更新时间或 Markdown 正文。决策聊天会读取规则、画像和看板，但当前不提供任何外部仓库写入能力。
